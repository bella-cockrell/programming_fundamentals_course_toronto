# This script was provided by the course to show how 
# I could use the functions I've written in a GUI.

import tkinter
import strings_and_designing_functions
import time
import math
...
for f in EXPECTED_FNS:
    if not hasattr(strings_and_designing_functions, f):
        print(f, "is missing in strings_and_designing_functions.py.")
        strings_and_designing_functions.__dict__[f] = lambda *x: 0
...
class Clock(object):
    ...
    def __init__(self, tk, zone=None, **kargs):
        ...
        self.diff = strings_and_designing_functions.hours_difference(dlt, 0)
    ...
    def refresh(self, forced=False):
        ...
        hour = int(strings_and_designing_functions.time_from_utc(utc_hour, self.diff))
        ...
        minutes_angle = strings_and_designing_functions.get_minutes(int(self.now)) / 60 * 360
        if strings_and_designing_functions.time_from_utc(0, self.diff) % 1 == 0.5:
            minutes_angle += 180
        elif strings_and_designing_functions.time_from_utc(0, self.diff) % 1 == 0.75:
            minutes_angle += 270
        ...
        seconds_angle = int(strings_and_designing_functions.get_seconds(int(self.now))) / 60 * 360
